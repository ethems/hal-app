...
const productController = (apiRouter) => {
  ...
  apiRouter.get('/products/withrate', (req, res) => {
    co(function * () {
      const productsWithRate = [];
      const products = yield Product.getAllWithActivePrice();
      for (let i = 0, len = products.length; i < len; i++) {
        const previousPrice = yield Price.getPreviousDayClosePrice(products[i].id, moment().startOf('day'));
        if (previousPrice && products[i].priceHistory) {
          const oldPrice = previousPrice[0];
          const newPrice = products[i].priceHistory[0];
